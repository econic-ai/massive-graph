...
/// Bloom filter with interior mutability for concurrent inserts/queries.
...
impl DeltaBloom {
    /// Create a bloom filter sized for target capacity and false-positive rate.
    ...
    /// Insert a prehashed key into the bloom filter.
    ...
    /// Check if a prehashed key might be in the set (may return false positives).
    ...
    /// Clear all bits in the bloom filter (reset to empty state).
    /// Safe to call concurrently with reads (readers may see partial state during clear).
    pub fn clear(&self) {
