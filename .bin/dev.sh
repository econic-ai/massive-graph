#!/bin/bash
# Development script with debugging enabled for Massive Graph Database
...
# Set development environment variables with debugging
export RUST_LOG=debug,massive_graph=trace
export MG_LOG_LEVEL=debug
export MG_DATA_DIR="./data"
export MG_HTTP_ADDR="${MG_HTTP_ADDR:-0.0.0.0:8080}"
export MG_WS_ADDR="127.0.0.1:8081"
export MG_QUIC_ADDR="127.0.0.1:8082"
export MG_WORKER_THREADS=2  # Lower for debugging

# Enable Rust debugging features
export RUST_BACKTRACE=full
export RUST_LIB_BACKTRACE=1

print_status "Starting Massive Graph Database in debug mode..."
...
# Build in debug mode with full debug info
print_status "Building in debug mode with full debug information..."
if ! cargo build; then
...
print_status "Starting Massive Graph Database with debugging enabled..."
print_warning "Press Ctrl+C to stop the server"
print_status "Full backtraces and detailed logging are enabled"

# Trap Ctrl+C to cleanup
trap 'print_status "Shutting down..."; exit 0' INT

# Run with cargo for better error messages and debugging symbols
cargo run 